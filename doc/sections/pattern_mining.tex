...
Searching specifically for rules having as consequent \emph{HasAwardsorNominations}, we found the following: \emph{((90.0, 3000.0]\_RuntimeMin, (4.19, 9.664]\_TotalCredits, (5.007, 13.782]\_NumVotes, movie\_TitleType, genre\_Drama, country\_Other} (\% support: 0.96, confidence: 0.82, lift: 4.9).
This rule identifies dramatic movies not produced exclusively in the US, UK, Japan or France that are popular and have productions of bigger dimensions.
Checking against our data, we were in fact able to confirm that these characteristics are a less reliable predictor for awards and nominations for titles produced in top countries (66\% vs 82\% for foreign movies).
This may indicate that foreign movies need critical acclaim to become popular, more so than movies produced in top countries.
...
\begin{table}
    ...
    \centering
    \rowcolors{2}{}{gray!25}
    \begin{tabular}{p{3cm}p{1.4cm}ccc}
    \toprule
...
\begin{table}
    ...
    \centering
    \rowcolors{2}{}{gray!25}
    \begin{tabular}{p{2.4cm}p{1.4cm}ccc}
    \toprule
    \textbf{Antecedent} & \textbf{Conseq.} & \textbf{\%sup} & \textbf{conf} & \textbf{lift} \\
    \midrule
...
The information about having episodes was enough to achieve perfect recall for tvSeries (P: 0.85, R: 1), but of course all tvMiniSeries were also classified as such. The same happens for tvShorts, all classified as shorts (P: 0.89, R: 0.91) --- the same had happened for our other classifiers. The information about \textit{runtimeMinutes} has lead to some misclassification, as the binning is quite coarse-grained; this explains the lower recall for tvEpisodes (P: 0.95, R: 0.57) and, especially, for movies (P: 0.81, R: 0.53): there's plenty of movies with a runtime < 90 minutes. All of our classification models have yielded better performances for these last two classes: the information carried by the other features proved to be useful.
