...
In this section, we will use the FP-Growth algorithm to extract frequent itemsets and association rules.
Preprocessing is required to turn the dataset into a list of transactions, with each record representing a transaction containing one item for each value the record takes for each feature. We have two multilabel variables, \textit{genres} and \textit{countryOfOrigin}, and we decided to only keep the four most frequent countries to reduce dimensionality, aggregating the rest into an 'Other' category. Numeric variables are discretized into quartile-based bins (as reported in Table \ref{tab:bin-intervals}), and, to allow interpretability, the name of the variables are appended to their values.
...
We keep the minimum number of items in the itemsets (\textit{zmin}) fixed to two, as they could already be interesting. Figure \ref{fig:max_closed_itemsets.png} shows the number of closed and maximal frequent itemsets extracted for different \textit{minsup} values, ranging from 2\% to 30\%. The number of itemsets drops quickly, and after 25\% the two groups seem to start converging to lower values (< 250). We also look at the maximal frequent itemsets that contain our target variable, \textit{titleType} (Figure \ref{fig:max_itemsets_titleType.png}): there are none for the minority classes tvShort, videoGame, tvMiniSeries, tvSpecial, and we start losing more classes from \textit{minsup} > 4\%.
...
After testing different \textit{minsup} values, we choose 30\%, as we get some interesting maximal itemsets, rather than just a combination of the most frequent values for the different features. At this threshold, the frequent itemsets are 174 and they are all closed, while the maximal itemsets are only 26. In Table \ref{tab:maximal_itemsets} are reported some of the most interesting maximal itemsets, among the top 10 with the highest support. 
...
The total number of rules extracted is 16466. The histograms in Figure \ref{fig:rules_conf_lift.png} show the rules' confidence and lift distributions. Given the high threshold, rules have high confidence, with the majority having \textit{conf} > 0.87. Nevertheless, the large majority is weak, with a lift < 1.
\begin{figure}[ht]
    \centering
...
For each of these four \textit{titleType} classes, we implement the rule with the highest support for a task of multiclass classification, for the sake of which the remaining classes are aggregated to 'other'. We favor the rules with the highest support because they are more general and include the most important items in the antecedent; nevertheless, we make sure that their lift is higher than 1. The selected rules are reported in Table \ref{tab:association_rules_titleType}. 
...
\end{table}
%COMPARE TO THE PERFORMANCE OF THE CLASSIFIERS IN CLASSIFICATION SECTION
The performance of our simple rule-based classifier on the test set is reported in Table \ref{tab:class_report_rules}. It achieves an accuracy of 60\% and a macro F1 score of 68\%, outperforming our baseline model, a stratified dummy classifier trained on the training set, across all metrics. 
\begin{table}[h]
    \centering
    \small
    \begin{tabular}{lcccc}
        ...
        \hline
        accuracy   &  &  & \textbf{0.600} & 5478 \\
        macro avg  & 0.741 & 0.698 & 0.\textbf{675} & 5478 \\
        \hline
    \end{tabular}
    \caption{Classification report for rule-based classification.}
    \label{tab:class_report_rules}
\end{table}
...
\begin{table}[h]
    ...
    \hline
    accuracy & & & \textbf{0.254} & 5478 \\
    macro avg & 0.204 & 0.204 & \textbf{0.204} & 5478 \\
    \hline
    \end{tabular}
    \caption{Classification report for the baseline classifier.}
    \label{tab:classification_report}
\end{table}

The confusion matrix in Figure \ref{fig:heatmap_classification_rules_tts.png} allows us to understand what types of mistakes the classifier made and how the 'other' classes were classified. 
...
\end{figure}
In conclusion, the association rules used leveraged on runtime, the information about serialized format, and the explicit information about shorts: these were exactly the top three most important features for our decision tree model.